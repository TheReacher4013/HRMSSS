...
// import { useNavigate } from "react-router-dom";
// import { Plus, Search, Edit2, Trash2, IndianRupee, X, Loader2 } from "lucide-react";
// // Sahi path check kar lein, aapke folders ke hisaab se
// import { advanceService } from "../../api/salaryService";
...
//     const [showModal, setShowModal] = useState(false);
//     const [editId, setEditId] = useState(null);
//     const [form, setForm] = useState({ name: "", amount: "", month: "", status: "Active" });
...
//         setLoading(true);
//         try {
//             const res = await advanceService.list();
//             setAllData(res.data);
//         } catch (err) {
...
//     const openAdd = () => {
//         setEditId(null);
//         setForm({ name: "", amount: "", month: "", status: "Active" });
//         setShowModal(true);
//     };
...
//             if (editId) {
//                 // Update Logic
//                 await advanceService.update(editId, form);
//             } else {
//                 // Create Logic
//                 await advanceService.create(form);
//             }
//             loadData(); // Data refresh karein
//             setShowModal(false);
//         } catch (err) {
...
//                                 <input placeholder="Search name..." value={search} onChange={e => { setSearch(e.target.value); setPage(1); }} className="pl-10 pr-4 py-2 bg-slate-50 border border-slate-200 rounded-xl text-sm w-full md:w-64 focus:ring-2 ring-emerald-50 outline-none transition-all" />
//                             </div>
//                             <button onClick={openAdd} className="bg-emerald-600 hover:bg-emerald-700 text-white p-2 md:px-4 rounded-xl flex items-center gap-2 transition-all active:scale-95 shadow-md">
//                                 <Plus size={18} /> <span className="hidden md:inline font-semibold">Add New</span>
//                             </button>
//                         </div>
//                     </div>
...
//                         <div className="p-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
//                             {pageData.map((r, i) => (
//                                 <div key={r._id} className="bg-[#fcfdfd] border border-slate-100 rounded-3xl p-5 hover:border-emerald-200 transition-all group relative overflow-hidden">
//                                     <div className="relative">
//                                         <div className="flex justify-between items-start mb-4">
//                                             <div className="bg-white p-2 rounded-xl shadow-sm border border-slate-100 text-slate-400 text-xs font-bold">#{startIndex + i + 1}</div>
//                                             <span className="text-[10px] font-black uppercase tracking-widest text-emerald-600 bg-emerald-50 px-2 py-1 rounded-lg">{r.status}</span>
...
//                                         </div>
//                                         <div className="flex gap-2">
//                                             <button onClick={() => { setEditId(r._id); setForm(r); setShowModal(true); }} className="flex-1 bg-white border border-slate-200 hover:border-blue-400 hover:text-blue-600 p-2.5 rounded-xl transition-all flex items-center justify-center gap-2 text-sm font-medium"><Edit2 size={14} /> Edit</button>
//                                             <button onClick={() => deleteRow(r._id)} className="p-2.5 rounded-xl bg-white border border-slate-200 hover:border-red-200 hover:bg-red-50 hover:text-red-600 transition-all"><Trash2 size={16} /></button>
//                                         </div>
//                                     </div>
//                                 </div>
//                             ))}
...
//                         <div className="p-6 border-t border-slate-50 flex justify-center gap-2">
//                             {Array.from({ length: totalPages }, (_, i) => (
//                                 <button key={i} onClick={() => setPage(i + 1)} className={`w-10 h-10 rounded-xl font-bold transition-all ${page === i + 1 ? "bg-[#0a4d44] text-white" : "bg-slate-50 text-slate-400"}`}>{i + 1}</button>
//                             ))}
//                         </div>
//                     )}
//                 </div>
...
//                                 <div className="space-y-1">
//                                     <label className="text-[10px] font-black uppercase text-slate-400 ml-1">Employee Name</label>
//                                     <input className="w-full bg-slate-50 border border-slate-200 p-3 rounded-xl outline-none" value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} placeholder="Full Name" />
//                                 </div>
//                                 <div className="grid grid-cols-2 gap-4">
//                                     <div className="space-y-1">
//                                         <label className="text-[10px] font-black uppercase text-slate-400 ml-1">Amount</label>
//                                         <input type="number" className="w-full bg-slate-50 border border-slate-200 p-3 rounded-xl outline-none" value={form.amount} onChange={e => setForm({ ...form, amount: e.target.value })} placeholder="0.00" />
//                                     </div>
//                                     <div className="space-y-1">
//                                         <label className="text-[10px] font-black uppercase text-slate-400 ml-1">Month</label>
//                                         <input type="month" className="w-full bg-slate-50 border border-slate-200 p-3 rounded-xl outline-none" value={form.month} onChange={e => setForm({ ...form, month: e.target.value })} />
//                                     </div>
//                                 </div>
//                                 <button onClick={saveRow} className="w-full bg-[#0a4d44] hover:bg-slate-800 text-white p-4 rounded-2xl font-bold transition-all mt-4">Confirm & Save</button>
//                             </div>
//                         </div>
//                     </div>
//                 )}
