...
import { hiringService } from '../../api/hiringService'; // Service import
...
const CandidateTable = () => {
    ...
    const [formData, setFormData] = useState({ _id: null, name: "", candidateId: "", phone: "" });

    const loadData = async () => {
        try {
            setLoading(true);
            const res = await hiringService.candidateList();
    ...
    const handleSave = async (e) => {
        ...
        try {
            if (formData._id) {
                await hiringService.candidateUpdate(formData._id, formData);
            } else {
                await hiringService.candidateCreate(formData);
            }
            setIsModalOpen(false);
            loadData();
    ...
    const handleDelete = async (id) => {
        if (window.confirm("Delete this candidate?")) {
            await hiringService.candidateDelete(id);
            loadData();
        }
    };
    ...
    return (
        <div className="p-4 md:p-8 bg-slate-50 min-h-screen">
            <div className="max-w-6xl mx-auto mb-8 flex flex-col sm:flex-row justify-between items-center gap-4">
                <div>
                    <h2 className="text-3xl font-black text-slate-800 tracking-tight">Candidate Directory</h2>
                    <p className="text-slate-500 font-medium">Manage your applicant pool</p>
                </div>
                <button onClick={() => { setFormData({ _id: null, name: "", candidateId: "", phone: "" }); setIsModalOpen(true); }}
                    className="bg-emerald-600 text-white px-6 py-3 rounded-xl font-bold flex items-center shadow-lg">
            ...
            <div className="bg-white rounded-[2rem] shadow-sm border overflow-hidden">
                <div className="p-6 bg-slate-50/50">
                    <div className="relative w-full md:w-96">
                        <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" size={18} />
                        <input type="text" placeholder="Filter by name..." className="w-full pl-12 pr-4 py-3 border rounded-2xl outline-none" onChange={(e) => setSearchTerm(e.target.value)} />
                    </div>
                </div>
                ...
                <table className="w-full text-left">
                    <thead className="bg-slate-50/80">
                        <tr>
                            <th className="p-5 text-[11px] font-black text-slate-400 uppercase">Candidate</th>
                            <th className="p-5 text-[11px] font-black text-slate-400 uppercase">System ID</th>
                            <th className="p-5 text-[11px] font-black text-slate-400 uppercase">Phone</th>
                            <th className="p-5 text-center text-[11px] font-black text-slate-400 uppercase">Actions</th>
                        </tr>
                    </thead>
                    <tbody>
                        {loading ? <tr><td colSpan="4" className="p-10 text-center">Loading...</td></tr> :
                            filtered.map((item) => (
                                <tr key={item._id} className="hover:bg-slate-50/50 group">
                                    <td className="p-5 font-bold text-slate-700">{item.name}</td>
                                    <td className="p-5 font-mono text-sm text-slate-500">{item.candidateId}</td>
                                    <td className="p-5 text-slate-600">{item.phone}</td>
                                    <td className="p-5 flex justify-center gap-2">
                                        <button onClick={() => { setFormData(item); setIsModalOpen(true); }} className="p-2 text-emerald-600"><Edit size={16} /></button>
                                        <button onClick={() => handleDelete(item._id)} className="p-2 text-rose-500"><Trash2 size={16} /></button>
                                    </td>
                                </tr>
                            ))}
                    </tbody>
            ...
            {/* Modal logic remains same, just ensure it uses handleSave */}
            {isModalOpen && (
                <div className="fixed inset-0 bg-slate-900/60 flex items-center justify-center z-50 p-4 backdrop-blur-md">
                    <div className="bg-white rounded-[2.5rem] w-full max-w-md p-8">
                        <h3 className="text-xl font-black mb-6">{formData._id ? "Update Profile" : "New Candidate"}</h3>
                        <form onSubmit={handleSave} className="space-y-4">
                            <input className="w-full border p-4 rounded-2xl" placeholder="Full Name" value={formData.name} onChange={e => setFormData({ ...formData, name: e.target.value })} required />
                            <input className="w-full border p-4 rounded-2xl" placeholder="System ID" value={formData.candidateId} onChange={e => setFormData({ ...formData, candidateId: e.target.value })} required />
                            <input className="w-full border p-4 rounded-2xl" placeholder="Phone" value={formData.phone} onChange={e => setFormData({ ...formData, phone: e.target.value })} required />
                            <button className="w-full bg-slate-900 text-white py-4 rounded-2xl font-black">Save Candidate</button>
                        </form>
                    </div>
                </div>
            )}
