...
import { hiringService } from '../../api/hiringService';

const CandidateSelection = () => {
    const [selections, setSelections] = useState([]);
    const [candidates, setCandidates] = useState([]); // Directory se data ke liye
    const [isModalOpen, setIsModalOpen] = useState(false);
    const [searchTerm, setSearchTerm] = useState("");
    const [formData, setFormData] = useState({ _id: null, name: "", candidateId: "", employeeId: "", position: "", terms: "Full Time" });

    const loadData = async () => {
        try {
            const [hiringRes, candidateRes] = await Promise.all([
                hiringService.hiringList(),
                hiringService.candidateList()
            ]);
            setSelections(hiringRes.data);
    ...
    const handleSave = async (e) => {
        e.preventDefault();
        if (formData._id) {
            await hiringService.hiringUpdate(formData._id, formData);
        } else {
            await hiringService.hiringCreate(formData);
        }
        setIsModalOpen(false);
        loadData();
    };
    ...
    return (
        <div className="p-4 md:p-8 bg-slate-50 min-h-screen">
            <div className="bg-white rounded-[2rem] p-8 border mb-6 flex justify-between items-center">
                <div className="flex items-center gap-4">
                    <Briefcase className="text-indigo-600" size={32} />
                    <h2 className="text-3xl font-black text-slate-900 italic">Hiring Board</h2>
                </div>
                <button onClick={() => { setFormData({ _id: null, name: "", candidateId: "", employeeId: "", position: "", terms: "Full Time" }); setIsModalOpen(true); }}
                    className="bg-indigo-600 text-white px-8 py-4 rounded-2xl font-black shadow-lg">
                    New Selection
                </button>
            </div>

            {/* Table and Filter logic same as your design, just mapping over 'selections' state */}
            {/* ... Desktop Table UI ... */}

            {/* Updated Modal with Dropdown */}
            {isModalOpen && (
                <div className="fixed inset-0 bg-slate-900/70 flex items-center justify-center z-50 p-4">
                    <form onSubmit={handleSave} className="bg-white rounded-[2rem] w-full max-w-xl p-8 grid grid-cols-2 gap-4">
                        <div className="col-span-2">
                            <label className="text-[10px] font-black uppercase text-slate-400">Select Candidate</label>
                            <select
                                className="w-full bg-slate-50 border p-4 rounded-2xl font-bold mt-1"
                                value={formData.name}
                                onChange={(e) => {
                                    const selected = candidates.find(c => c.name === e.target.value);
                                    setFormData({ ...formData, name: selected.name, candidateId: selected.candidateId });
                                }}
                                required
                            >
                                <option value="">-- Choose From Directory --</option>
                                {candidates.map(c => <option key={c._id} value={c.name}>{c.name} ({c.candidateId})</option>)}
                            </select>
                        </div>

                        <input className="border p-4 rounded-2xl" placeholder="Employee ID" value={formData.employeeId} onChange={e => setFormData({ ...formData, employeeId: e.target.value })} required />
                        <input className="border p-4 rounded-2xl" placeholder="Position" value={formData.position} onChange={e => setFormData({ ...formData, position: e.target.value })} required />

                        <select className="col-span-2 border p-4 rounded-2xl" value={formData.terms} onChange={e => setFormData({ ...formData, terms: e.target.value })}>
                            <option>Full Time</option><option>Contract</option><option>Probation</option><option>Remote</option>
                        </select>

                        <button type="submit" className="col-span-2 bg-indigo-600 text-white py-5 rounded-2xl font-black">Confirm Selection</button>
                    </form>
                </div>
            )}
        </div>
