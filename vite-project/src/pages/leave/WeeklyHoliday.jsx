...
import { Pencil, Trash2, Plus, Calendar as CalendarIcon, Settings, CheckCircle, XCircle, X, Loader2 } from "lucide-react";
// Humne apni service file import kar li
import {
  getHolidays, updateHolidays,
...
const WeeklyHoliday = () => {
  ...
  // States
  const [holidays, setHolidays] = useState([]); // Will fetch from DB
  const [editingHoliday, setEditingHoliday] = useState(null);
  const [selectedDays, setSelectedDays] = useState([]);
  ...
  const fetchData = async () => {
    ...
    try {
      if (activeTab === "weekly") {
        ...
        // Backend object deta hai, humein array chahiye loop ke liye
        setHolidays(res.data ? [res.data] : []);
      } else if (activeTab === "leaveType") {
        const res = await getLeaveTypes();
        setLeaveTypes(res.data);
      } else if (activeTab === "leaveApproval") {
  ...
  const handleUpdateHoliday = async () => {
    try {
      await updateHolidays(selectedDays);
      fetchData(); // Refresh data
      setEditingHoliday(null);
    } catch (err) { alert("Failed to update holidays"); }
  };

  // --- 3. LEAVE POLICY ACTIONS ---
  const handleSaveLeave = async () => {
    try {
      if (editLeave) {
        await updateLeaveType(editLeave._id, leaveForm);
      } else {
  ...
  const handleDeletePolicy = async () => {
    try {
      await deleteLeaveType(deleteLeaveId);
  ...
  const handleStatusChange = async (id, status) => {
    try {
      // Backend expects status in 'form' body or specific status update
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="flex flex-wrap gap-2 bg-white/50 backdrop-blur p-2 rounded-[2rem] border border-slate-100 mb-8 inline-flex shadow-sm">
          {["weekly", "leaveType", "leaveApproval"].map((tab) => (
            <button
              key={tab}
              onClick={() => setActiveTab(tab)}
              className={`px-6 py-3 rounded-[1.5rem] text-sm font-black transition-all ${activeTab === tab ? "bg-[#0a4d44] text-white shadow-md scale-105" : "text-slate-400 hover:text-slate-600"}`}
            >
              {tab === 'weekly' ? 'Weekly Off' : tab === 'leaveType' ? 'Policy Types' : 'Approvals'}
            </button>
          ))}
        ...
        {loading ? (
          ...
          <>
            {/* WEEKLY HOLIDAY CONTENT */}
            {activeTab === "weekly" && (
              <div className="bg-white p-8 rounded-[3rem] shadow-sm border border-slate-100 animate-in fade-in duration-500">
                <h2 className="text-xl font-black text-slate-800 mb-6 flex items-center gap-2"><CalendarIcon size={20} /> Office Holidays</h2>
                <div className="space-y-6">
                  {holidays.length > 0 ? holidays.map((item) => (
                    <div key={item._id} className="p-6 bg-slate-50 rounded-[2rem] border border-slate-100 flex flex-col md:flex-row justify-between items-center gap-4">
                      <div className="flex flex-wrap gap-2 justify-center md:justify-start">
                        {item.days.map(day => (
                          <span key={day} className="px-4 py-1.5 bg-white border border-slate-200 rounded-xl text-xs font-bold text-slate-600 shadow-sm">{day}</span>
                        ))}
                  ...
                  )) : <p className="text-slate-400 text-center py-10">No Holidays Set</p>}
                  ...
                  {editingHoliday && (
                    <div className="mt-8 pt-8 border-t border-slate-100 animate-in slide-in-from-top duration-300">
                      ...
                      <div className="flex flex-wrap gap-3 mb-8">
                        {daysList.map((day) => (
                          <button key={day} onClick={() => setSelectedDays(prev => prev.includes(day) ? prev.filter(d => d !== day) : [...prev, day])} className={`px-5 py-3 rounded-2xl text-xs font-bold transition-all border ${selectedDays.includes(day) ? "bg-[#0a4d44] border-emerald-900 text-white shadow-lg" : "bg-white border-slate-100 text-slate-400 hover:border-slate-300"}`}>{day}</button>
                        ))}
                      </div>
                      <div className="flex gap-3">
                        <button onClick={handleUpdateHoliday} className="bg-slate-800 text-white px-8 py-3 rounded-2xl font-bold text-sm">Save Changes</button>
                        <button onClick={() => setEditingHoliday(null)} className="text-slate-400 px-6 py-3 font-bold text-sm">Cancel</button>
                      </div>
                    </div>
                  )}
                </div>
            ...
            {activeTab === "leaveType" && (
              <div className="bg-white p-8 rounded-[3rem] shadow-sm border border-slate-100 animate-in fade-in duration-500">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {leaveTypes.map((item) => (
                    <div key={item._id} className="p-6 bg-slate-50 border border-slate-100 rounded-[2.5rem] flex justify-between items-center group hover:bg-white hover:shadow-xl transition-all">
                      <div>
                        <span className="text-[10px] font-black text-indigo-500 uppercase tracking-widest">{item.code}</span>
                        <h3 className="font-bold text-slate-800">{item.name}</h3>
                        <p className="text-xs text-slate-400 font-bold mt-1 tracking-tight">{item.days} Days Yearly</p>
                      ...
                      <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-all">
                        <button onClick={() => { setEditLeave(item); setLeaveForm(item); setShowLeaveModal(true); }} className="p-2.5 bg-white border border-slate-100 rounded-xl text-indigo-600 hover:bg-indigo-50"><Pencil size={14} /></button>
                        <button onClick={() => setDeleteLeaveId(item._id)} className="p-2.5 bg-white border border-slate-100 rounded-xl text-red-500 hover:bg-red-50"><Trash2 size={14} /></button>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            {activeTab === "leaveApproval" && (
              <div className="bg-white p-8 rounded-[3rem] shadow-sm border border-slate-100 animate-in fade-in duration-500">
                <h2 className="text-xl font-black text-slate-800 mb-8">Pending Approvals</h2>
                <div className="space-y-4">
                  {leaveRequests.filter(req => req.status === "pending").map((item) => (
                    <div key={item._id} className="p-6 bg-slate-50/50 border border-slate-100 rounded-[2rem] flex flex-col md:flex-row justify-between items-center gap-4">
                      <div className="flex items-center gap-4">
                        <div className="w-12 h-12 bg-white rounded-2xl border border-slate-100 flex items-center justify-center text-indigo-600 font-black">{item.employeeid?.charAt(0) || "E"}</div>
                        <div>
                          <h4 className="font-bold text-slate-800">{item.employeeid}</h4>
                          <p className="text-xs text-slate-400 font-bold uppercase tracking-tighter">{item.type} • Reason: {item.leaveReason}</p>
                        </div>
                      </div>
                      <div className="flex gap-2">
                        <button onClick={() => handleStatusChange(item._id, "Approved")} className="px-6 py-2.5 bg-white text-emerald-600 border border-emerald-100 rounded-xl font-black text-xs hover:bg-emerald-600 hover:text-white transition-all">Approve</button>
                        <button onClick={() => handleStatusChange(item._id, "Rejected")} className="px-6 py-2.5 bg-white text-red-500 border border-red-100 rounded-xl font-black text-xs hover:bg-red-50 transition-all">Reject</button>
                      </div>
                    </div>
                  ))}
                  {leaveRequests.filter(req => req.status === "pending").length === 0 && <p className="text-center text-slate-400 py-10 font-bold">No Pending Requests</p>}
                </div>
              </div>
            )}
          </>
        ...
        {showLeaveModal && (
          <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-[9999] flex justify-center items-center p-4">
            <div className="bg-white w-full max-w-sm rounded-[3rem] p-8 shadow-2xl">
              <h2 className="text-xl font-black text-slate-800 mb-6">{editLeave ? "Edit Policy" : "New Policy"}</h2>
              <div className="space-y-4 mb-8">
                <input placeholder="Policy Name" value={leaveForm.name} onChange={(e) => setLeaveForm({ ...leaveForm, name: e.target.value })} className="w-full bg-slate-50 border border-slate-100 px-6 py-4 rounded-2xl text-sm outline-none" />
                <div className="grid grid-cols-2 gap-4">
                  <input placeholder="Code (e.g. SL)" value={leaveForm.code} onChange={(e) => setLeaveForm({ ...leaveForm, code: e.target.value })} className="w-full bg-slate-50 border border-slate-100 px-6 py-4 rounded-2xl text-sm outline-none" />
                  <input type="number" placeholder="Days" value={leaveForm.days} onChange={(e) => setLeaveForm({ ...leaveForm, days: e.target.value })} className="w-full bg-slate-50 border border-slate-100 px-6 py-4 rounded-2xl text-sm outline-none" />
                </div>
              ...
              <div className="flex gap-2">
                <button onClick={() => setShowLeaveModal(false)} className="flex-1 py-4 font-bold text-slate-400">Close</button>
                <button onClick={handleSaveLeave} className="flex-[2] bg-[#0a4d44] text-white py-4 rounded-2xl font-bold">Save Policy</button>
              </div>
            </div>
          </div>
        )}
        ...
        {deleteLeaveId && (
          <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-md z-[9999] flex justify-center items-center p-4">
            <div className="bg-white w-full max-w-sm rounded-[3rem] p-8 text-center shadow-2xl">
              <div className="w-20 h-20 bg-red-50 rounded-full flex items-center justify-center mx-auto mb-6 text-red-500"><Trash2 size={40} /></div>
              <h2 className="text-xl font-black text-slate-800 mb-2">Delete Policy?</h2>
              <button onClick={handleDeletePolicy} className="w-full bg-red-500 text-white py-4 rounded-2xl font-bold shadow-lg shadow-red-100 mb-2">Delete Forever</button>
              <button onClick={() => setDeleteLeaveId(null)} className="w-full py-4 rounded-2xl font-bold text-slate-400">Go Back</button>
            </div>
          </div>
        )}
