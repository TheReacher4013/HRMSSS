...
import { Pencil, Trash2, Building2, Plus, AlertCircle, X, Loader2 } from "lucide-react";
import { useAuth } from "../../context/AuthContext"; // AuthContext ka path check karlein
...
export default function Department() {
  ...
  const saveDepartment = async () => {
    ...
    try {
      if (current) {
        // Update Request
        const res = await axios.put(`http://localhost:5000/api/departments/${current._id}`,
          { name: deptName, status },
          { headers: { Authorization: `Bearer ${token}` } }
        );
        setList((prev) => prev.map((d) => (d._id === current._id ? res.data : d)));
      } else {
        // Create Request
  ...
  const confirmDelete = async () => {
    try {
      await axios.delete(`http://localhost:5000/api/departments/${current._id}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      setList((prev) => prev.filter((d) => d._id !== current._id));
      setDeleteOpen(false);
    } catch (err) {
      alert("Delete failed");
    }
  ...
  /* ---------- Pagination & Modals Setup ---------- */
  const totalPages = Math.ceil(list.length / ITEMS_PER_PAGE);
  const paginatedData = list.slice((page - 1) * ITEMS_PER_PAGE, page * ITEMS_PER_PAGE);

  const onAdd = () => { setCurrent(null); setDeptName(""); setStatus("Active"); setModalOpen(true); };
  const onEdit = (item) => { setCurrent(item); setDeptName(item.name); setStatus(item.status); setModalOpen(true); };
    ...
    <div className="bg-[#f8fafc] min-h-screen p-4 md:p-8 pt-24">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
          ...
          <button onClick={onAdd} className="w-full md:w-auto bg-[#0a4d44] hover:bg-slate-800 text-white px-6 py-3 rounded-2xl flex items-center justify-center gap-2 transition-all shadow-lg font-bold active:scale-95">
            <Plus size={20} /> Add Department
          </button>
        </div>
        ...
        {loading && list.length === 0 ? (
          ...
          <>
              ...
              <table className="w-full text-left">
                <thead className="bg-slate-50/50 border-b border-slate-100">
                  <tr>
                    ...
                    <th className="px-6 py-4 text-[11px] font-black uppercase text-slate-400">Department Name</th>
                ...
                <tbody className="divide-y divide-slate-50">
                  {paginatedData.map((d, i) => (
                    <tr key={d._id} className="hover:bg-slate-50/50 transition-colors group">
                      ...
                      <td className="px-6 py-5">
                        <div className="flex justify-center gap-2">
                          <button onClick={() => onEdit(d)} className="p-2.5 bg-white border border-slate-100 text-blue-600 rounded-xl hover:border-blue-200 transition-all"><Pencil size={16} /></button>
                          <button onClick={() => onDelete(d)} className="p-2.5 bg-white border border-slate-100 text-red-600 rounded-xl hover:border-red-200 transition-all"><Trash2 size={16} /></button>
                        </div>
                      </td>
                    </tr>
                  ))}
            ...
            {/* Mobile Cards View & Pagination yahan same rahega... */}
          </>
        )}

        {/* DELETE & ADD MODALS logic with loading states... */}
      </div>
    </div>
  );
}